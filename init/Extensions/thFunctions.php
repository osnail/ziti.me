...
/**
 * 返回去用户首页的链接
 * 如www.ziti.me/c=userblog&a=index&domain=home&uid=1
 **/
...
spAddViewFunction('goUserHome','goUserHome'); //将此函数添加入模板引擎 
...
{
	$site_uri = trim(dirname($GLOBALS['G_SP']['url']["url_path_base"]),"\/\\"); //获取站点根目录
		...
		foreach($feeds as $d)
		{
			...
			if($d['btype'] == 1)//文字
			{
				...
				<a class="boxhover" href="'.goUserBlog(array('bid'=>$d['bid'])).'" target="_blank">
					<div class="img"><img src="'.avatar(array('uid'=>$d['uid'],'size'=>'middle')).'" alt=""/></div>
			...
			{
			 $msg .= '
				    <div class="box"  style="background:url('.$d['img'].') no-repeat center top;">
					<div class="imgcount"> '.$d['count'].' 张</div> <span class="tag">'._setTags('照片',$d['tag'],$index).'</span>
					<a class="boxhover" href="'.goUserBlog(array('bid'=>$d['bid'])).'" target="_blank">
					<div class="img"><img src="'.avatar(array('uid'=>$d['uid'],'size'=>'middle')).'" alt=""/></div>
					<div class="p">来自:'.$d['username'].'</div>
					</a>
			...
			if($d['btype'] == 4) //视频
			{
				if($d['metype'] == 'local')
				  ...
				  <div class="box">
					...
					<div class="video"> <img src="'.$site_uri.'/tpl/image/videoplay.gif" alt=""/> </div>
				'.$d['desc'].'
				<a class="boxhover" href="'.goUserBlog(array('bid'=>$d['bid'])).'" target="_blank">
				<div class="img"><img src="'.avatar(array('uid'=>$d['uid'],'size'=>'middle')).'"alt="" /></div>
					<div class="p">来自:'.$d['username'].'</div>
		...
		if(is_array($data))
		{
			foreach($data as $d)
			{
				...
				if($d['type'] == 1) //文字
				{
					...
					$rsdata[] = array('bid'=>$d['bid'],'btype'=>$d['type'],'img'=>$img,'content'=>$content,
									 'uid'=>$d['user']['uid'],'username'=>$d['user']['username'],'domian'=>$d['user']['domain'],'tag'=>$d['tag']);
					...
					$rsdata[] = array('bid'=>$d['bid'],'btype'=>$d['type'],'metype'=>$midetype,'img'=>$img,'pid'=>$pid,'desc'=>$desc,
										'uid'=>$d['user']['uid'],'username'=>$d['user']['username'],'domian'=>$d['user']['domain'],'tag'=>$d['tag']);
					...
					if($rs['attr']['img'])
					{
						...
						$rsdata[] = array('bid'=>$d['bid'],'btype'=>$d['type'],'img'=>$img,'count'=>$count,
										'uid'=>$d['user']['uid'],'username'=>$d['user']['username'],'domian'=>$d['user']['domain'],'tag'=>$d['tag']);
					}
				}
			}
